 ...
 * @api public
 */
var Nog = exports = module.exports = function Nog(level, stream, options) {
    ...
    log: function(levelStr, args) {
        if (exports[levelStr] <= this.level) {
            ...
            if (this.options.filename) {
                var path = __stack[3].getFileName();
                if(path) {
                    var paths = path.split("/");
                    var filename = paths[paths.length-1];
                this.stream.write(filename + ' ');
                }
            }
            if (this.options.line_column) {
                this.stream.write("["+ __stack[3].getLineNumber()+ ':'+ __stack[3].getColumnNumber()+"] ");
            }
