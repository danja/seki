(function() {
 ...
 // NB : 'save' is now a tick icon
 buttonClasses: { // 'heading',  , 'image' 'clear-formatting',
      'default': ['save'],
 'all': ['bold', 'italic', 'underline', 'unordered-list', 'ordered-list', 'link', 'save'],
  ...
  _.extend(etch, {
    ...
    editableInit: function(e) {
      ...
      $('body').bind('mousedown', function(e) { // editor.mousedown
      ...
      //  console.log("target = "+$(target));
        if ($(target).not('.etch-editor-panel, .etch-editor-panel *, .etch-image-tools, .etch-image-tools *').size()) {
          // remove editor
         // $editor.remove(); MUST CLICK tick TO CLOSE
